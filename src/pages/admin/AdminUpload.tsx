...
function inferQuestionType(q: any) {
  const rawType = String(q.type || "").trim();
  const text = String(q.text || "").trim();
  const normalizedType = rawType.toLowerCase().replace(/\s+/g, "-");
  if (/^assertion[- ]?reason$/i.test(rawType) || /assertion\s*[:\-]/i.test(text) && /reason\s*[:\-]/i.test(text)) {
    return "Assertion-Reason";
  }
  if (/^case[- ]?based$/i.test(rawType)) return "Case-Based";
  if (/^mcq$/i.test(rawType) || !rawType) return "MCQ";
  return rawType;
}
...
export default function AdminUpload() {
  ...
  const applyResult = (result: any) => {
    ...
    const ds: EditableQuestion[] = (result.questions || []).map((q: any) => ({
      ...
      // null when the parser found no answer key — do NOT assume option A.
