...
export type EditableQuestion = Partial<Question> & {
  ...
  text: string;
  options: string[];
  /** null = no answer key. Never coerce to 0. */
...
export function emptyQuestion(): EditableQuestion {
  return {
    text: "",
    options: ["", "", "", ""],
    // A brand-new question has no answer key until the admin picks one.
    correctIndex: null,
    subject: "Physics",
    topic: "",
    difficulty: "Moderate",
          ...
          {value.options.map((opt, i) => (
            <div key={i} className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  // Clicking the marked option again clears it, so an admin can
                  // return a question to "no answer key" instead of being stuck.
                  onClick={() => update({ correctIndex: value.correctIndex === i ? null : i })}
