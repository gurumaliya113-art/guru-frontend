...
function isCorrect(q: Question, ans: string | undefined | null): boolean {
  ...
  // No answer key => the attempt can never be graded as correct. Previously
  // this fell back to options[0], silently grading option A as the answer.
  if (q.correctIndex == null) return false;
  if (isNumerical(q)) {
    const correct = q.options?.[q.correctIndex] ?? "";
    if (!String(correct).trim()) return false;
